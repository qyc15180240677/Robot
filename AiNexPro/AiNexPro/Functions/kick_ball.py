...
x_2 = 320
y_2 = y_dis + 300
x_pid = PID(P=0.03, I=0.01, D=0.002)#pid初始化
y_pid = PID(P=0.03, I=0.01, D=0.002)
...
def move():    
    ...
    while True:
        if __isRunning and not debug:
            if ball_status == 1:
                count = 0
                if 125 <= x_dis < x_center - 60:#不在中心，x_center为中心
                    if y_dis >= 320:
                        if x_dis < x_center - 150:
                            print(1)
                    ...
                    if y_dis > y_min or Y <= 340:
                        AGC.runActionGroup(go_forward)
                    elif y_dis <= y_min and 480 >= Y > 340:
                        if 875 > x_dis > x_center + 60 or x_center + 20 > x_dis > x_center - 20:
                            AGC.runActionGroup(left_move)
                        elif x_center - 60 > x_dis > 125:
                            AGC.runActionGroup(right_move)
                        elif x_dis > x_center:
                            AGC.runActionGroup(left_shot)
                        elif x_dis <= x_center:
                            AGC.runActionGroup(right_shot)
                ball_status = 2
